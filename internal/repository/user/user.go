...
func (ur *UserRepository) CreateNewUser(uuid, token string) error {
	...
	sqlStatement := "INSERT INTO USERS (guid,hashed_refresh_token,expires_at) VALUES ($1,$2,$3)"
	_, err = tx.Exec(context.Background(), sqlStatement, uuid, token, time.Now().Add(time.Hour*48))
	if err != nil {
...
func (u *UserRepository) FindUserByUUID(uuid string) (*models.User, error) {
	sqlStatement := "SELECT u.guid, u.hashed_refresh_token FROM USERS as u where u.guid = $1"
	var user models.User
	err := u.db.QueryRow(context.Background(), sqlStatement, uuid).Scan(&user.GUID, &user.HashedRefreshToken)
	if err != nil {
